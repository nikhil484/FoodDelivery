  ...
  return (
    ...
    <div className='w-full min-h-screen bg-[#fff9f6] flex flex-col items-center'>
        ...
        <div className='flex justify-center items-center p-4 sm:p-6'>
         ...
         p-6 border border-gray-100 hover:shadow-xl transition-shadow duration-300'>
            <div className='flex flex-col items-center text-center'>
             ...
             hover:bg-orange-600 transition-colors duration-200'onClick={() => navigate('/create-edit-shop')}>
                Get Started
              </button>
            </div>
          </div>
      ...
      <div className='w-full flex flex-col items-center gap-6 px-4 sm:px-6'>
       <h1 className='rext-2xl sm:text-3xl text-gray-900 flex items-center gap-3 mt-8 text-center'>Welcome to {myShopData.name}
       ...
       <div className='bg-white shadow-lg rounded-xl overflow-hidden border border-orange-100 hover:shadow-2xl transition-all
        duration-300 w-full max-w-3xl relative'>
           <div className='absolute top-4 right-4 bg-[#ff2d4d] text-white p-2 rounded-full shadow-md
      ...
      {myShopData.items.length==0 &&  
        <div className='flex justify-center items-center p-4 sm:p-6'>
         ...
         p-6 border border-gray-100 hover:shadow-xl transition-shadow duration-300'>
            <div className='flex flex-col items-center text-center'>
              ...
              <button className='bg-[#ff4d2d] text-white px-5 sm:px-6 py-2 rounded-full font-medium shadow-md
             hover:bg-orange-600 transition-colors duration-200'onClick={() => navigate('/add-item')}>
                Add Food
              </button>
            </div>
          </div>
        ...
        className='flex flex-col items-center gap-4 w-full max-w-3xl'>
          {myShopData.items.map((item,index)=>(<OwnerItemCard data={item} key={index}/>))}</div>}
      </div>
      }
    </div>
  )
